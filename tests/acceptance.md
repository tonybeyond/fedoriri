...
| 8 | Shadow démarre, /dev/uinput accessible, clavier/souris répondent | oui | **VALIDÉ** (2026-08-21, matériel) : Shadow 9.9.10457 se lance (fenêtre « Shadow PC » vue par niri) une fois `fuse-libs` présent (libfuse.so.2 exigé par l'AppImage — ajouté au pool) ; `/dev/uinput` = `crw-rw---- root:shadow-input`, fedo ∈ input,shadow-input (effectif après reconnexion). Clavier/souris dans une session Shadow : à confirmer par l'utilisateur (compte requis) | `ls -l /dev/uinput` (groupe shadow-input, 0660), `id fedo` (groupes shadow-input,input), lancer Shadow |
